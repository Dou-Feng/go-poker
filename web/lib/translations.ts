export type Language = "en" | "zh";

export const translations = {
  en: {
    sendChatMessage: "Send message",
    quickReplies: "Quick reactions",
    resizeChat: "Resize chat",
    expandChat: "Expand chat",
    collapseChat: "Collapse chat",
    increaseBuyIn: "Increase buy-in",
    decreaseBuyIn: "Decrease buy-in",
    lobbyTools: "Lobby tools",
    lobbyEmptyTitle: "No rooms yet",
    lobbyEmptyHint: "Create a room and invite your friends to play",
    lobbyTagline: "Find a table and play together",
    roomTagline: "Texas Hold’em · Play together",
    createFirstRoom: "Create the first room",
    moreSettings: "More settings",
    roomAutoName: "Leave blank for a random name",
    optionalField: "Optional",
    smallBlindLabel: "Small",
    bigBlindLabel: "Big",
    historyDetailsHint: "Select a record to view game details",

    authMode: "Account access",
    authUserId: "User ID",
    authChooseAvatar: "Choose your avatar",
    authAvatar: "Avatar",
    authOtherMethods: "Or continue with",
    authUnavailable: "Social login coming soon",

    loadingAssets: "Loading assets",
    title: "GoPoker",
    username: "username",
    uuid: "uuid",
    uuidHint: "at least 5 letters & digits",
    identifier: "username or uuid",
    password: "password",
    signUp: "sign up",
    logIn: "log in",
    lobby: "Lobby",
    chips: "chips",
    recharge: "Recharge",
    walletBalance: "Balance",
    connectionUnavailable: "Server unavailable. Still reconnecting…",
    connecting: "Connecting to server…",
    reconnecting: "Connection interrupted. Reconnecting…",
    connectionLost:
      "Unable to reconnect. Returned to the lobby; still retrying.",
    addChips1000: "+1000 chips",
    logout: "logout",
    loggedInAs: "logged in as",
    newRoomName: "new room name",
    newRoom: "New room",
    create: "create",
    rooms: "rooms",
    refresh: "refresh",
    noRooms: "no rooms yet — create one to start playing",
    players: "players",
    player: "player",
    watching: "watching",
    running: "running",
    join: "join",
    cancel: "cancel",
    roomPassword: "room password",
    friends: "friends",
    addFriend: "add friend",
    friendUuid: "friend uuid",
    myStats: "my stats",
    viewStats: "view stats",
    leave: "leave",
    reset: "reset",
    start: "Start",
    spectating: "👁 Spectating",
    pickSeat: "Pick a seat to play",
    spectatorPanel: "Spectating",
    spectatorList: "Spectators",
    noSpectators: "No spectators yet",
    guestSpectator: "Guest",
    you: "You",
    seatReservedStatus: "Reserved for next hand",
    seatReservedShort: "Reserved",
    autoSeatNextHand: "You'll be seated after this hand",
    reserveNextHandHint: "Reserve a seat for the next hand",
    takeSeatNow: "Take a seat",
    myReservedSeat: "My reserved seat",
    joinNextHandShort: "Joining next hand",
    winner: "Winner",
    hand_royal_flush: "Royal flush",
    hand_straight_flush: "Straight flush",
    hand_four_of_a_kind: "Four of a kind",
    hand_full_house: "Full house",
    hand_flush: "Flush",
    hand_straight: "Straight",
    hand_three_of_a_kind: "Three of a kind",
    hand_two_pair: "Two pair",
    hand_one_pair: "One pair",
    hand_high_card: "High card",
    chipsForfeited: "chips forfeited",
    call: "call",
    bet: "bet",
    check: "check",
    fold: "fold",
    close: "close",
    min: "min",
    halfPot: "1/2 pot",
    threeQuarterPot: "3/4 pot",
    twoPot: "2× pot",
    pot: "pot",
    allIn: "all in",
    buyIn: "Buy In",
    buyInLabel: "buy-in",
    maxBuy: "Max buy-in",
    net: "net",
    rebuy: "rebuy",
    tapToRebuy: "Tap to add chips",
    tapToRecharge: "Tap to top up",
    handsPlayed: "hands played",
    winRate: "win rate",
    foldRate: "fold rate",
    threeBetRate: "3bet rate",
    vpip: "VPIP",
    gameBehavior: "Game behavior",
    vpipByPosition: "VPIP by position",
    positionStatsHint:
      "Only hands dealt to 5+ players count. — means no qualifying data.",
    posBTN: "Button",
    posSB: "Small blind",
    posBB: "Big blind",
    posUTG: "Under the gun",
    posMP: "Middle position",
    posCO: "Cutoff",
    maxPotWon: "max pot won",
    calls: "calls",
    raises: "raises",
    chat: "chat",
    log: "Hand history",
    logStartHand: "New hand starting",
    logSmallBlind: "{0} posts the small blind ({1})",
    logBigBlind: "{0} posts the big blind ({1})",
    logFolds: "{0} folds",
    logChecks: "{0} checks",
    logCalls: "{0} calls",
    logCallsAmount: "{0} calls {1}",
    logBets: "{0} bets {1}",
    logRaisesTo: "{0} raises to {1}",
    logAllIn: "{0} is all in",
    logWins: "{0} wins {1}",
    logChipsForfeited: "chips forfeited",
    logBuysIn: "{0} buys in for {1}",
    logSitsDown: "{0} sits down at seat {1} for {2}",
    logTimedOutLeft: "{0} timed out and left the table",
    logTimeoutFold: "{0} ran out of time and folds",
    logTimeoutCheck: "{0} ran out of time and checks",
    logOutOfChips: "{0} is out of chips and moves to the spectators",
    logVotedSettle: "{0} voted to settle ({1}/{2})",
    logCancelledVote: "{0} cancelled their settle vote ({1}/{2})",
    logSettleApproved: "Early settlement approved — settling after this hand",
    saySomething: "say something...",
    total: "total",
    nlTexasHoldem: "nl texas holdem",
    table: "table",
    open: "Open",
    waitingToJoin: "Waiting to join",
    mustHaveTwoPlayers: "Must have 2 or more players to start game",
    viewRoomStats: "view room stats",
    switchLanguage: "中文",
    changeAvatar: "change avatar",
    changeUsername: "change username",
    newUsername: "new username",
    change: "change",
    uploadImage: "upload image",
    fileTooLarge: "file must be under 10MB",
    uploadFailed: "upload failed",
    noFriends: "no friends yet",
    add: "add",
    blinds: "blinds",
    buyIns: "buy-ins",
    maxPlayers: "players",
    invalidUsernameOrPassword: "invalid username or password",
    accountLoggedInElsewhere: "account logged in on another device",
    usernameUnavailable: "Username already taken",
    invalidCredentials: "Invalid credentials",
    invalidUsername: "invalid username",
    notLoggedIn: "not logged in",
    usernameNotUnique: "username is not unique — log in with your uuid",
    invalidUuid: "uuid must be at least 5 letters and digits",
    uuidAlreadyTaken: "uuid already taken",
    alreadyFriends: "already friends",
    roomAlreadyExists: "room already exists",
    notEnoughChips: "not enough chips",
    needChipsToReady: "not enough chips — add chips to play",
    wrongPassword: "wrong password",
    roomClosed: "that room has closed — back to the lobby",
    couldNotCreateRoom: "Could not create room",
    couldNotLoadUser: "could not load user",
    couldNotSaveUser: "could not save user",
    amountMustBePositive: "amount must be positive",
    cannotRebuyDuringHand: "cannot rebuy during a hand",
    youAreNotSeated: "you are not seated",
    history: "history",
    noHistory: "no games yet",
    alreadySeated: "already seated",
    maxBuyInReached: "max buy-in reached",
    ready: "Ready",
    readyStatus: "Ready",
    cancelReady: "Cancel ready",
    clickReadyButton: "Tap the ready button to start",
    waitingForPlayers: "Waiting for players",
    waitingForReady: "Waiting for ready",
    gatherPlayersPrefix: "Gather",
    gatherPlayersSuffix: "players to start",
    allPlayersReadyHint: "Everyone ready? Let's play!",
    inviteFriends: "Invite friends",
    invitationCopied: "Invitation copied",
    roomInvitation: "Join me on GoPoker! Find room:",
    copyInvitationManually: "Copy this invitation and send it to your friends",
    undo: "undo",
    cannotUndoDuringHand: "cannot undo during a hand",
    cannotUndoAfterReady: "cannot undo after readying up",
    gameAlreadyRunning: "game already running",
    alreadyInRoom: "Leave your current room before joining another",
    couldNotRefundPlayers: "Some refunds failed. Please retry the room reset",
    settlementPending:
      "Settlement is waiting for storage. Retrying automatically",
    gameNotRunning: "game not running",
    tableIsFull: "table is full",
    tapSeatToJoinNext: "Tap an empty seat to join the next hand",
    reservedNextHand: "Seat reserved — you join next hand (tap it to cancel)",
    nextHand: "Next hand",
    rebuyNextHand:
      "Rebuy accepted — the chips are added when the next hand is dealt",
    reserveSeat: "Reserve for next hand",
    cancelReservation: "Cancel reservation",
    hands: "hands",
    actionTimeout: "Turn timer (s)",
    unlimited: "unlimited",
    roomInfo: "Room info",
    roomName: "Room name",
    roomMode: "Game mode",
    modeCash: "Cash game · unlimited chips",
    roomCreated: "Created",
    roomPasswordYes: "Password required",
    roomPasswordNo: "No password",
    roomTurnClock: "Turn timer",
    detailStats: "Detailed stats",
    detailStatsHint: "Full hand statistics",
    achievements: "Achievements",
    comingSoon: "Coming soon",
    messageNotSupported: "Messaging is not supported yet",
    messageLabel: "Message",
    goBack: "Back",
    winnerLabel: "Winner",
    totalNet: "Total P/L",
    settlementPlayers: "Players",
    endChips: "End",
    settlementBack: "Leave room",
    settlementNext: "Next hand",
    settlement: "Settlement",
    biggestPot: "Biggest pot",
    earlySettle: "Early settle",
    voteSettle: "Surrender",
    showCards: "Show",
    spectate: "Spectate",
    roomStats: "Stats",
    noPlayers: "no players",
    settings: "Settings",
    language: "Language",
    voice: "Voice",
    sfx: "Sound effects",
    bgm: "Background music",
    male: "Male",
    female: "Female",
    voiceChat: "Voice chat",
    micOn: "Mic on — tap to mute",
    micOff: "Mic off — tap to talk",
    speakerOn: "Voice on — tap to stop listening",
    speakerOff: "Voice off — tap to listen",
    micVolume: "Mic input volume",
    muteAudio: "Mute",
    unmuteAudio: "Unmute",
    micProcessingDescription: "Reduce echo and keep your voice clear.",
    noiseCancellationDescription:
      "Filter background noise so your voice comes through.",
    micVolumeHint:
      "100% keeps the original level; raise it if your voice is too quiet.",
    voiceSettingsFailed:
      "Could not change microphone settings. The previous setting was kept.",
    othersVolume: "Others' voice volume",
    echoCancellation: "Echo & mic processing",
    noiseCancellation: "AI noise cancellation",
    noiseCancellationHint:
      "Filters keyboard, fans and background chatter with a local speech model (downloaded on first use, runs on this device). Replaces the browser's own noise suppression.",
    noiseCancellationFailed:
      "Could not start AI noise cancellation — browser noise suppression is still on",
    on: "On",
    off: "Off",
    more: "More",
    boardView: "Show board and hole cards",
    rank: "Rank",
    sessionSettled: "settled",
    sessionOpen: "in progress",
    tapPlayerForStats: "Tap a player to see their stats for this session",
    sessionNotFound: "record not found",
    addBot: "Bot",
    removeBot: "Remove bot",
    botModeDone: "Done placing bots",
    botModeHint: "Tap an empty seat to add a bot, tap a bot to remove it",
    noBots: "no bots at the table",
    hostOnly: "only the room host can manage bots",
    cannotAddBotReady: "cannot add bot while ready",
    seatTaken: "that seat is taken",
    tournament: "Tournament",
    tournamentHint:
      "cap total buy-ins per player; busted players are benched. Off = unlimited rebuys",
    botTypeAI: "AI bots",
    botTypeAIHint:
      "bots decide with the Deep CFR model; needs the AI service to be up",
    aiBotsUnavailable: "AI bot service unavailable",
    aiBotsTooManyPlayers: "AI bots support at most 6 players",
    unknownBotType: "unknown bot type",
    noBuyInsLeft: "no buy-ins left in this room",
    bustedOut: "you are out of chips and buy-ins — moved to the spectators",
    muteMicFor: "Mute this player for me",
    unmuteMicFor: "Unmute this player",
    micDenied: "microphone unavailable — check browser permissions",
    micNeedsHttps:
      "microphone needs a secure page — open the site over https (listening still works)",
    voiceUnsupported: "voice chat is not supported in this browser",
    voiceUnavailable: "voice chat is unavailable on this server",
    voiceConnectFailed:
      "could not reach the voice server — check that it is running and its port is open",
    isFriend: "friends",
  },
  zh: {
    sendChatMessage: "发送消息",
    quickReplies: "快捷表情和短语",
    resizeChat: "调整聊天窗口高度",
    expandChat: "展开聊天",
    collapseChat: "收起聊天",
    increaseBuyIn: "增加补码金额",
    decreaseBuyIn: "减少补码金额",
    lobbyTools: "大厅工具",
    lobbyEmptyTitle: "暂无房间",
    lobbyEmptyHint: "创建一个房间，邀请好友开始游戏",
    lobbyTagline: "找个房间，一起玩牌",
    roomTagline: "德州扑克 · 欢乐对局",
    createFirstRoom: "创建第一个房间",
    moreSettings: "更多设置",
    roomAutoName: "留空自动生成",
    optionalField: "选填",
    smallBlindLabel: "小盲",
    bigBlindLabel: "大盲",
    historyDetailsHint: "点击记录查看对局详情",

    authMode: "注册或登录",
    authUserId: "用户ID",
    authChooseAvatar: "选择你的头像",
    authAvatar: "头像",
    authOtherMethods: "或使用其他方式",
    authUnavailable: "第三方登录暂未开放",

    loadingAssets: "正在加载资源",
    title: "GoPoker",
    username: "用户名",
    uuid: "用户ID",
    uuidHint: "至少 5 位数字或字母",
    identifier: "用户名或 UUID",
    password: "密码",
    signUp: "注册",
    logIn: "登录",
    lobby: "大厅",
    chips: "筹码",
    recharge: "充值",
    walletBalance: "余额",
    connectionUnavailable: "暂时无法连接服务器，正在继续重试…",
    connecting: "正在连接服务器…",
    reconnecting: "网络连接中断，正在重连…",
    connectionLost: "重连超时，已返回大厅，正在继续尝试连接。",
    addChips1000: "+1000 筹码",
    logout: "退出登录",
    loggedInAs: "已登录",
    newRoomName: "新房间名",
    newRoom: "新建房间",
    create: "创建",
    rooms: "房间",
    refresh: "刷新",
    noRooms: "暂无房间 — 创建一个开始游戏",
    players: "玩家",
    player: "玩家",
    watching: "观看",
    running: "进行中",
    join: "加入",
    cancel: "取消",
    roomPassword: "房间密码",
    friends: "好友",
    addFriend: "添加好友",
    friendUuid: "好友 UUID",
    myStats: "我的战绩",
    viewStats: "查看战绩",
    leave: "离开",
    reset: "重开",
    start: "开始",
    spectating: "👁 观战中",
    pickSeat: "选个座位入座",
    spectatorPanel: "观战中",
    spectatorList: "观战列表",
    noSpectators: "暂无观战玩家",
    guestSpectator: "游客",
    you: "你",
    seatReservedStatus: "已预约下一局",
    seatReservedShort: "已预约",
    autoSeatNextHand: "将在本局结束后自动入座",
    reserveNextHandHint: "预约座位，下一局加入",
    takeSeatNow: "立即入座",
    myReservedSeat: "我的预订座位",
    joinNextHandShort: "下一局加入",
    winner: "赢家",
    hand_royal_flush: "皇家同花顺",
    hand_straight_flush: "同花顺",
    hand_four_of_a_kind: "四条",
    hand_full_house: "葫芦",
    hand_flush: "同花",
    hand_straight: "顺子",
    hand_three_of_a_kind: "三条",
    hand_two_pair: "两对",
    hand_one_pair: "一对",
    hand_high_card: "高牌",
    chipsForfeited: "筹码已消失",
    call: "跟注",
    bet: "加注",
    check: "过牌",
    fold: "弃牌",
    close: "关闭",
    min: "最小",
    halfPot: "1/2 底池",
    threeQuarterPot: "3/4 底池",
    twoPot: "2 倍底池",
    pot: "满池",
    allIn: "all in",
    buyIn: "买入",
    buyInLabel: "买入",
    maxBuy: "买入上限",
    net: "盈亏",
    rebuy: "补码",
    tapToRebuy: "点击补码",
    tapToRecharge: "点击充值",
    handsPlayed: "手数",
    winRate: "胜率",
    foldRate: "弃牌率",
    threeBetRate: "3bet率",
    vpip: "入池率",
    gameBehavior: "牌局行为",
    vpipByPosition: "按位置入池率",
    positionStatsHint: "仅统计发牌时至少 5 人参与的牌局；— 表示暂无有效数据。",
    posBTN: "按钮位",
    posSB: "小盲",
    posBB: "大盲",
    posUTG: "枪口",
    posMP: "中位",
    posCO: "关煞位",
    maxPotWon: "最大底池",
    calls: "跟注次数",
    raises: "加注次数",
    chat: "聊天",
    log: "牌局记录",
    logStartHand: "新一局开始",
    logSmallBlind: "{0} 下小盲注（{1}）",
    logBigBlind: "{0} 下大盲注（{1}）",
    logFolds: "{0} 弃牌",
    logChecks: "{0} 过牌",
    logCalls: "{0} 跟注",
    logCallsAmount: "{0} 跟注 {1}",
    logBets: "{0} 下注 {1}",
    logRaisesTo: "{0} 加注到 {1}",
    logAllIn: "{0} 全押",
    logWins: "{0} 赢得 {1}",
    logChipsForfeited: "筹码已没收",
    logBuysIn: "{0} 买入 {1}",
    logSitsDown: "{0} 入座 {1} 号位，买入 {2}",
    logTimedOutLeft: "{0} 超时离场",
    logTimeoutFold: "{0} 超时，自动弃牌",
    logTimeoutCheck: "{0} 超时，自动过牌",
    logOutOfChips: "{0} 筹码耗尽，移至观战",
    logVotedSettle: "{0} 投票结算（{1}/{2}）",
    logCancelledVote: "{0} 取消了结算投票（{1}/{2}）",
    logSettleApproved: "提前结算已通过，本局结束后结算",
    saySomething: "说点什么...",
    total: "总额",
    nlTexasHoldem: "无限注德州扑克",
    table: "桌子",
    open: "空位",
    waitingToJoin: "等待加入",
    mustHaveTwoPlayers: "至少需要 2 名玩家才能开始",
    viewRoomStats: "查看本局战绩",
    switchLanguage: "English",
    changeAvatar: "更换头像",
    changeUsername: "修改用户名",
    newUsername: "新用户名",
    change: "修改",
    uploadImage: "上传图片",
    fileTooLarge: "图片不能超过 10MB",
    uploadFailed: "上传失败",
    noFriends: "暂无好友",
    add: "添加",
    blinds: "盲注",
    buyIns: "买入次数",
    maxPlayers: "人数",
    invalidUsernameOrPassword: "用户名或密码错误",
    accountLoggedInElsewhere: "账号已在其他设备登录",
    usernameUnavailable: "用户名已被注册",
    invalidCredentials: "用户名或密码错误",
    invalidUsername: "无效的用户名",
    notLoggedIn: "未登录",
    usernameNotUnique: "用户名不唯一，请使用 UUID 登录",
    invalidUuid: "UUID 需至少 5 位数字或字母",
    uuidAlreadyTaken: "UUID 已被注册",
    alreadyFriends: "已经是好友",
    roomAlreadyExists: "房间已存在",
    notEnoughChips: "筹码不足",
    needChipsToReady: "筹码不足，请先补充筹码",
    wrongPassword: "密码错误",
    roomClosed: "原房间已关闭，已返回大厅",
    couldNotCreateRoom: "无法创建房间",
    couldNotLoadUser: "无法加载用户",
    couldNotSaveUser: "无法保存用户",
    amountMustBePositive: "金额必须为正数",
    cannotRebuyDuringHand: "牌局进行中不能补码",
    youAreNotSeated: "你还没有入座",
    history: "历史战绩",
    noHistory: "暂无对局记录",
    alreadySeated: "你已经入座了",
    maxBuyInReached: "已达最大买入上限",
    ready: "准备",
    readyStatus: "已准备",
    cancelReady: "取消准备",
    clickReadyButton: "点击准备按钮开始",
    waitingForPlayers: "等待玩家加入",
    waitingForReady: "等待玩家准备",
    gatherPlayersPrefix: "凑齐",
    gatherPlayersSuffix: "名玩家即可开始",
    allPlayersReadyHint: "所有玩家准备后即可开始",
    inviteFriends: "邀请好友",
    invitationCopied: "邀请信息已复制",
    roomInvitation: "来 GoPoker 一起玩！在大厅找到房间：",
    copyInvitationManually: "复制以下邀请信息，发送给好友",
    undo: "撤回",
    cannotUndoDuringHand: "牌局进行中不能撤回",
    cannotUndoAfterReady: "已准备，不能撤回",
    gameAlreadyRunning: "牌局已开始",
    alreadyInRoom: "请先退出当前房间，再加入其他房间",
    couldNotRefundPlayers: "部分筹码返还失败，请重试重置房间",
    settlementPending: "结算暂未完成，正在自动重试，请稍候",
    gameNotRunning: "牌局尚未开始",
    tableIsFull: "桌子已满",
    tapSeatToJoinNext: "点击空位加入下一局",
    reservedNextHand: "已预订座位，下一局入座（再点该座位可取消）",
    nextHand: "下一局",
    rebuyNextHand: "补码成功，筹码将在下一局发牌时到账",
    reserveSeat: "预订下一局",
    cancelReservation: "取消预订",
    hands: "局数",
    actionTimeout: "操作时限（秒）",
    unlimited: "不限",
    roomInfo: "房间信息",
    roomName: "房间名称",
    roomMode: "模式",
    modeCash: "现金局 · 无限筹码",
    roomCreated: "创建时间",
    roomPasswordYes: "有密码",
    roomPasswordNo: "无密码",
    roomTurnClock: "操作时限",
    detailStats: "详细数据",
    detailStatsHint: "更完整的牌局统计",
    achievements: "成就徽章",
    comingSoon: "敬请期待",
    messageNotSupported: "发消息功能暂未开放",
    messageLabel: "发消息",
    goBack: "返回",
    winnerLabel: "本局赢家",
    totalNet: "总盈亏",
    settlementPlayers: "本局玩家",
    endChips: "结束",
    settlementBack: "退出房间",
    settlementNext: "继续下一局",
    settlement: "结算",
    biggestPot: "最大底池",
    earlySettle: "提前结算",
    voteSettle: "投降",
    showCards: "秀牌",
    spectate: "观战",
    roomStats: "战绩",
    noPlayers: "暂无玩家",
    settings: "设置",
    language: "语言",
    voice: "音色",
    sfx: "音效",
    bgm: "背景音乐",
    male: "男声",
    female: "女声",
    voiceChat: "语音",
    micOn: "麦克风已开 — 点击关闭",
    micOff: "麦克风已关 — 点击开麦",
    speakerOn: "语音已开 — 点击停止收听",
    speakerOff: "语音已关 — 点击收听他人",
    micVolume: "麦克风输入音量",
    muteAudio: "静音",
    unmuteAudio: "取消静音",
    micProcessingDescription: "减少回声，让语音更清晰。",
    noiseCancellationDescription: "过滤环境杂音，让对方听清你的声音。",
    micVolumeHint: "100% 为原始音量；对方听不清时可调高。",
    voiceSettingsFailed: "无法修改麦克风设置，已保留原设置。",
    othersVolume: "他人语音音量",
    echoCancellation: "麦克风增强",
    noiseCancellation: "AI 降噪",
    noiseCancellationHint:
      "本地语音模型过滤键盘声、风扇与背景人声（首次开启时下载，在本机运行）。会取代浏览器自带的降噪。",
    noiseCancellationFailed: "AI 降噪启动失败，已继续使用浏览器降噪",
    on: "开",
    off: "关",
    more: "更多",
    boardView: "查看公牌与底牌",
    rank: "名次",
    sessionSettled: "已结算",
    sessionOpen: "进行中",
    tapPlayerForStats: "点击玩家查看其本局数据",
    sessionNotFound: "找不到该牌局记录",
    addBot: "Bot",
    removeBot: "移除机器人",
    botModeDone: "完成",
    botModeHint: "点击空位添加机器人，点击机器人将其移除",
    noBots: "牌桌上没有机器人",
    hostOnly: "只有房主可以管理机器人",
    cannotAddBotReady: "准备状态下不能添加机器人",
    seatTaken: "该座位已被占用",
    tournament: "锦标赛",
    tournamentHint: "限制每人总买入，输光者转观战；关闭则不限补码",
    botTypeAI: "AI 机器人",
    botTypeAIHint: "机器人由 Deep CFR 模型驱动，需要 AI 服务在线",
    aiBotsUnavailable: "AI 机器人服务不可用",
    aiBotsTooManyPlayers: "AI 机器人最多支持 6 人牌局",
    unknownBotType: "未知的机器人类型",
    noBuyInsLeft: "本房间的买入次数已用完",
    bustedOut: "你已输光且没有剩余买入次数，已转为观战",
    muteMicFor: "屏蔽此人语音（仅对我）",
    unmuteMicFor: "取消屏蔽此人语音",
    micDenied: "无法使用麦克风，请检查浏览器权限",
    micNeedsHttps:
      "麦克风需要 https 页面 — 请用 https 打开本站（收听不受影响）",
    voiceUnsupported: "当前浏览器不支持语音",
    voiceUnavailable: "此服务器未开启语音功能",
    voiceConnectFailed: "无法连接语音服务器，请确认它已启动且端口已放行",
    isFriend: "已是好友",
  },
} as const;

export type TranslationKey = keyof typeof translations.en;

// Maps English server/client error strings to a translation key.
export const errorKeyByMessage: Record<string, TranslationKey> = {
  "invalid username or password": "invalidUsernameOrPassword",
  "account logged in on another device": "accountLoggedInElsewhere",
  "username already taken": "usernameUnavailable",
  "username and password required": "invalidUsernameOrPassword",
  "could not hash password": "couldNotSaveUser",
  "could not save user": "couldNotSaveUser",
  "could not load user": "couldNotLoadUser",
  "room already exists": "roomAlreadyExists",
  "wrong password": "wrongPassword",
  "room closed": "roomClosed",
  "not enough chips": "notEnoughChips",
  "not enough chips to ready": "needChipsToReady",
  "already friends": "alreadyFriends",
  "amount must be positive": "amountMustBePositive",
  "cannot rebuy during a hand": "cannotRebuyDuringHand",
  "you are not seated": "youAreNotSeated",
  "Username unavailable": "usernameUnavailable",
  "Invalid credentials": "invalidCredentials",
  "invalid username": "invalidUsername",
  "not logged in": "notLoggedIn",
  "username not unique": "usernameNotUnique",
  "invalid uuid": "invalidUuid",
  "uuid already taken": "uuidAlreadyTaken",
  "Could not create room": "couldNotCreateRoom",
  "already seated": "alreadySeated",
  "max buy-in reached": "maxBuyInReached",
  "not in a room": "youAreNotSeated",
  "cannot undo during a hand": "cannotUndoDuringHand",
  "cannot undo after ready": "cannotUndoAfterReady",
  "game already running": "gameAlreadyRunning",
  "already in a room": "alreadyInRoom",
  "could not refund players": "couldNotRefundPlayers",
  "settlement pending, retrying automatically": "settlementPending",
  "game not running": "gameNotRunning",
  "table is full": "tableIsFull",
  "no buy-ins left": "noBuyInsLeft",
  "no bots at the table": "noBots",
  "record not found": "sessionNotFound",
  "only the host can manage bots": "hostOnly",
  "cannot add bot while ready": "cannotAddBotReady",
  "seat is taken": "seatTaken",
  "AI bot service unavailable": "aiBotsUnavailable",
  "AI bots support at most 6 players": "aiBotsTooManyPlayers",
  "unknown bot type": "unknownBotType",
  "busted: no buy-ins left": "bustedOut",
};
