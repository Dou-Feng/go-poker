...
Backend (Go 1.26, module `github.com/evanofslack/go-poker`, run from `backend/`):
...
- **Client-driven advancement**: the server never uses timers to advance a hand, with one opt-in exception, the per-room action clock (`server/clock.go`): when a room is created with `actionTimeout` seconds, every broadcast arms a deadline for the turn in the view (same turn keeps its deadline, a new turn gets a fresh one), and on expiry the server checks the player to act when legal, otherwise folds them, then broadcasts. `update-game` carries `actionTimeout` and `actionRemainingMs` (time left, not an absolute time, so client clock skew is irrelevant); the acting seat draws the countdown bar. The frontend (`web/components/Table.tsx`) elects one "runout driver" (first non-left player) whose browser sends `deal-game` after the showdown/runout animation delays. If that client is gone, the table stalls until another `deal-game` arrives.
...
- **Joining mid-hand is a seat reservation** (`server/reserve.go`): `take-seat` while `view.Running` toggles a per-account claim on that seat (sent as `reserved` in `update-game`, drawn by `Seat.tsx`); nobody is seated during a hand. `seatReservedPlayers` converts claims into seats when the stage is back to `NotReady` (called from `broadcastGame` and, before `autoStartIfReady`, in `handleDealGame`'s showdown branch) and leaves the newcomer **not ready**, so the table waits for them instead of auto-dealing. There is no "join next hand" button or `queue-next` action any more. Wallet access in that path goes through `table.userStore()` (test hook `table.users`).
...
- **Voice chat runs on LiveKit; the server only mints access tokens** (`server/livekit.go`). All WebRTC media flows through one self-hosted LiveKit SFU (the `livekit` compose service, **opt-in** via the `voice` profile: `COMPOSE_PROFILES=voice` + `LIVEKIT_API_KEY`/`LIVEKIT_API_SECRET` in `.env`; host networking, ports 7880/7881/tcp + a UDP media range). `get-livekit-token` (logged-in, table-attached clients only) returns a short-lived JWT signed with the API pair: identity = `accountUUID`, name = username, room = `gopoker-<tablename>`, audio publish/subscribe only (microphone source, no data channels). Without the key pair the reply is `ok:false` and the client shows "voice unavailable". The client side (`web/lib/voice.ts`, singleton) keeps the custom mic chain (getUserMedia → gain/limiter WebAudio graph → published as a `LocalAudioTrack`) plus mic/speaker toggles and per-peer local mute; remote audio plays through per-identity `<audio>` elements. `Game.tsx` binds it to the room, `WebSocket.tsx` feeds it the socket and `livekit-token` replies. Tokens are requested on room entry/first activation and refreshed near expiry (`LIVEKIT_TOKEN_TTL_HOURS`, 6 h); unexpected disconnects reconnect after 1 s.
...
- Styling: the brand surface ladder (`brand/lobby/floor/table/tablehi/card/cardhi` + text `ink`/`muted`) is defined in `tailwind.config.js` — never hand-roll gray/blue surfaces. Buttons compose `btn` + one variant (`btn-primary|confirm|accent|secondary|ghost|danger|text|icon`) and text uses the semantic roles (`type-display|heading|label|caption|num`; body text is just `text-ink`/`text-muted`), both from `@layer components` in `styles/index.css` — card faces and seat markers stay raw utilities. The in-game chrome is plain CSS loaded from `pages/_app.tsx`: action keys are layered-material buttons (`styles/actionbar.css`, `.gp-action-bar` / `.gp-action-btn--check|bet|allin|fold`, PNG layers under `public/assets/ui/buttons/`; `InputButton` picks the kind) shown only on the player's own turn, the raise panel above them is `styles/raisepanel.css` (`.gp-raise-panel`, `RaiseInput.tsx`), and occupied seats are `styles/seat.css` (`.gps-seat` + `--idle|hero|active|folded|allin|winner|left|removable`, sized by custom properties for phone/desktop; D/SB/BB are avatar badges). Card faces, empty seats and seat markers stay Tailwind utilities. The login doodle wallpaper (`.login-wallpaper`) and the in-app doodle wallpaper (`.room-wallpaper`, lobby + game room) with their WebP derivatives live in `public/bg-*.webp` with sources + regeneration commands in `web/assets-src/README.md`. The poker-table surfaces are UE-style materials (`.felt-material`/`.rail-material`): fixed-density tiling textures — the oval felt/rail geometry stays CSS shape (`rounded-full` + padding) and the texture never stretches.
