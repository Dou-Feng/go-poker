...
- Styling: the brand surface ladder (`brand/lobby/floor/table/tablehi/card/cardhi` + text `ink`/`muted`) is defined in `tailwind.config.js` — never hand-roll gray/blue surfaces. Buttons compose `btn` + one variant (`btn-primary|confirm|accent|secondary|ghost|danger|text|icon`) and text uses the semantic roles (`type-display|heading|label|caption|num`; body text is just `text-ink`/`text-muted`), both from `@layer components` in `styles/index.css` — card faces and seat markers stay raw utilities. Game action keys use `action-key` + one of `action-key-call|bet|allin|fold|neutral` (same layer; `InputButton` picks the variant) and the bar renders only on the player's own turn. The login doodle wallpaper (`.login-wallpaper`) and the in-app doodle wallpaper (`.room-wallpaper`, lobby + game room) with their WebP derivatives live in `public/bg-*.webp` with sources + regeneration commands in `web/assets-src/README.md`. The poker-table surfaces are UE-style materials (`.felt-material`/`.rail-material`): fixed-density tiling textures — the oval felt/rail geometry stays CSS shape (`rounded-full` + padding) and the texture never stretches.
